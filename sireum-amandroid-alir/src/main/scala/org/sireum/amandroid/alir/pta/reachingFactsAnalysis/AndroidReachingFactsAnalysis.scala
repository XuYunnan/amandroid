...
class AndroidReachingFactsAnalysisBuilder(clm : ClassLoadManager){
  ...
  def buildGeneral //
  (entryPointProc : JawaProcedure,
      existingIdfg : InterProceduralDataFlowGraph,
      existingIrfaResult : InterProceduralMonotoneDataFlowAnalysisResultExtended[RFAFact],
   initialFacts : ISet[RFAFact] = isetEmpty,
   timer : Option[MyTimer],
   switchAsOrderedMatch : Boolean) : (InterProceduralDataFlowGraph, AndroidReachingFactsAnalysisExtended.Result)  = {
    val gen = new Gen
    val kill = new Kill
    val callr = new Callr
    val nl = new NodeL
    val initial : ISet[RFAFact] = isetEmpty
    val idfg = if(existingIdfg==null) {
      val newicfg = new InterproceduralControlFlowGraph[ICFGNode]
      val newptares = new PTAResult
            ...
            case (x, y) => {
              x match {
                case x1:ICFGEntryNode =>
                  val proc = x1.getOwner
                  rpcMethods.foreach{
                    rm =>
                      if(rm.getSignature == proc){
                        val callers = rpcData.getCallersByCallee(rm)
                        val callfacts = callers.map { x => x.callFacts }.foldLeft(Set[RFAFact]())(_ union _)
    ...
    (InterProceduralDataFlowGraph(icfg, ptaresult), result)
  }
...
//      }
        ...
        callee =>
          ...
          if(AndroidReachingFactsAnalysisHelper.isICCCall(calleep) || AndroidReachingFactsAnalysisHelper.isModelCall(calleep)){
            ...
            if(rpcCallFlag){ // for a RPC-call           
              ...
              caller.callFacts ++= reducedNewFacts
              rpcData.add(calleep, caller)
    ...
    def getAndMapFactsForCaller(calleeS : ISet[RFAFact], callerNode : ICFGNode, calleeExitNode : ICFGVirtualNode) : ISet[RFAFact] ={
      ...
      val resultForRpcCaller: MSet[RFAFact] = msetEmpty
      resultForRpcCaller ++= result
      callerNode match{
...
object AndroidReachingFactsAnalysisExtended {
  ...
  def apply(entryPointProc : JawaProcedure,
   ...
   clm : ClassLoadManager,
   timer : Option[MyTimer],
   switchAsOrderedMatch : Boolean = false) : (InterProceduralDataFlowGraph, Result)
           = new AndroidReachingFactsAnalysisBuilder(clm).buildGeneral(entryPointProc, existingIdfg, existingIrfaResult,initialFacts, timer, switchAsOrderedMatch)
}