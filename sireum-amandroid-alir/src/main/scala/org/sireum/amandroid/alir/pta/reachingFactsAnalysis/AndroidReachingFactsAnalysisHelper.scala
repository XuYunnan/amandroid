  ...
  def doIrfaMerge(entryPoints:Set[JawaProcedure], parallel: Boolean, timer : Option[MyTimer]) ={    
    ...
    if(phase2){
      while(!worklist.isEmpty){
        ...
        val callfacts = if(!irfaResult.getExtraInfo.getRpcData.getCalleeCallers.keySet.isEmpty)
            irfaResult.getExtraInfo.getRpcData.getCallersByCallee(irfaResult.getExtraInfo.getRpcData.getCalleeCallers.keySet.head).head.callFacts.toString()
            else " "
        val retfacts = if(!irfaResult.getExtraInfo.getRpcData.getCalleeCallers.keySet.isEmpty)
            irfaResult.getExtraInfo.getRpcData.getCallersByCallee(irfaResult.getExtraInfo.getRpcData.getCalleeCallers.keySet.head).head.retFacts.toString()
            else " "
        System.out.println("at end of convergence loop: irfaResult.getExtraInfo.getRpcData callfacts = " + callfacts)
        System.out.println(" and retfacts = " + retfacts)
